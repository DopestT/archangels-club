/**
 * CreatorDashboard — Prototype A: "Command Center"
 *
 * Wide single-column layout. Cinematic hero with creator avatar.
 * 4 stat tiles in a row. Split layout: earnings chart left / activity feed right.
 * Content grid below. Payout card pinned at top when not set up.
 */
...
export default function CreatorDashboardV1() {
  ...
  async function startStripeOnboarding() {
    ...
    try {
      ...
      if (res.status === 401) { setStripeError('Session expired. Please refresh the page and try again.'); setStripeLoading(false); return; }
      const data = await res.json();
      if (!res.ok) { setStripeError(data.error ?? 'Payout setup unavailable. Please try again shortly.'); setStripeLoading(false); return; }
      if (data.url) { window.location.href = data.url; }
      else { setStripeError('Payout setup unavailable. Please try again shortly.'); setStripeLoading(false); }
    } catch (err) {
    ...
    <>
      ...
      <div className="min-h-screen bg-bg-primary py-10 xl:py-14">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
          ...
          {/* ── PAYOUT CTA — pinned at very top when not set up ─────────────────── */}
          {stripeStatus !== null && !stripeStatus.onboarded && (
            <div
              className="flex items-start gap-4 p-5 rounded-2xl mb-8"
              style={{
                background: 'linear-gradient(135deg, rgba(212,175,55,0.10) 0%, rgba(20,20,25,0.98) 60%)',
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-serif text-base text-white mb-0.5">Enable payouts to start earning</p>
                <p className="text-xs text-arc-secondary leading-relaxed">
                  Connect your bank via Stripe. You keep <span className="text-gold font-medium">70%</span> of every payment — processed automatically each week.
                </p>
              ...
              <button
                ...
                className="btn-gold text-xs px-5 py-2.5 flex-shrink-0 flex items-center gap-2"
              >
                {stripeLoading ? (
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, rgba(212,175,55,0.08) 0%, rgba(10,10,15,0.98) 65%)',
          ...
          >
            <div
              ...
              style={{ background: 'radial-gradient(ellipse 55% 80% at 15% 50%, rgba(212,175,55,0.09) 0%, transparent 70%)' }}
            />
            <div className="relative px-8 py-10 sm:px-12 sm:py-14 xl:px-16">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-7 mb-8">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h1 className="font-serif text-4xl sm:text-5xl xl:text-6xl text-white leading-none mb-2">
                    {getGreeting()}, <em className="not-italic text-gold">{firstName}.</em>
                  </h1>
                  <p className="font-serif text-lg xl:text-xl italic leading-snug" style={{ color: 'rgba(212,175,55,0.72)' }}>
                ...
                {user?.username && (
                  <div className="hidden xl:flex flex-col items-end gap-2">
                    <span className="flex items-center gap-1.5 text-xs text-arc-muted">
                      <span
                        className="w-1.5 h-1.5 rounded-full"
                        style={{
                          background: isVerifiedCreator ? '#10B981' : 'rgba(255,255,255,0.2)',
                          boxShadow: isVerifiedCreator ? '0 0 6px rgba(16,185,129,0.5)' : 'none',
                        }}
                      />
                      {isVerifiedCreator ? 'Studio live' : 'Pending approval'}
                    </span>
                    {isVerifiedCreator && (
                      <span className="text-[10px] text-arc-muted">archangelsclub.com/creator/{user.username}</span>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-6" style={{ borderTop: '1px solid rgba(212,175,55,0.1)' }}>
                ...
                <button
                  ...
                  className="text-xs px-4 py-2.5 rounded-xl border border-white/12 text-arc-secondary hover:text-white hover:border-white/25 transition-all flex items-center gap-1.5"
                >
                  {profileLinkCopied ? <Check className="w-3.5 h-3.5 text-arc-success" /> : <Copy className="w-3.5 h-3.5" />}
          ...
          {profileSetupNeeded && !trainingViewed && (
            <div className="card-surface p-6 rounded-xl mb-6 border-gold/20">
          ...
          {stripeStatus?.onboarded && !hasEarnings && (
            <div className="flex items-center gap-3 p-4 rounded-xl bg-arc-success/8 border border-arc-success/25 mb-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-arc-muted mt-0.5">You receive 70% of every sale — deposited automatically each week.</p>
              </div>
            ...
            <div className="mb-10">
              ...
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-arc-secondary text-sm leading-relaxed max-w-sm mx-auto mb-6">Your first drop is one upload away.</p>
            ...
            <div className="card-surface p-6 rounded-xl">
              <div className="flex items-center justify-between mb-5">
                ...
                {stripeStatus?.onboarded ? (
                  <span className="flex items-center gap-1.5 text-[10px] font-medium text-arc-success">
                    <span className="w-1.5 h-1.5 rounded-full bg-arc-success flex-shrink-0" style={{ boxShadow: '0 0 5px rgba(16,185,129,0.55)' }} />
                    Payouts active
                  </span>
                ) : (
                  <button
              ...
              {transactions.length > 0 ? (
                          ...
                          {recent.map((t, i) => {
                            ...
                            return (
                              <div
                                ...
                                className="rounded-sm min-w-0 transition-all hover:opacity-100"
                              />
                            );
                          })}
                ...
                <div className="py-8 flex flex-col items-center gap-3 text-center">
                  <div className="flex items-end gap-1 h-10 opacity-15 w-full max-w-[140px]">
                    {[30, 50, 35, 65, 40, 55, 45, 70, 50, 80].map((h, i) => (
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                ...
                {activityFeed.length > 0 && (
                  <span className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-arc-success flex-shrink-0" style={{ animation: 'pulse 2.2s ease-in-out infinite' }} />
              ...
              {activityFeed.length > 0 ? (
                <div>
                  {activityFeed.slice(0, 8).map((a, i) => (
                    <div
                      key={i}
                      className={`flex items-start gap-3 px-6 py-3.5 ${i < Math.min(activityFeed.length, 8) - 1 ? 'border-b border-white/4' : ''}`}
                    >
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-8 ${!showContent ? 'hidden' : ''}`}>

            {/* Main col: custom requests + coaching */}
                    ...
                    {requests.map((req) => (
                      <div key={req.id} className="p-4 rounded-xl bg-bg-hover border border-white/5">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          ...
                          <span className="font-serif text-gold text-lg flex-shrink-0">{formatCurrency(Number(req.offered_price))}</span>
                        </div>
            ...
            <div className="space-y-5">
                ...
                <div className="px-5 pt-5 pb-3">
                  ...
                  <div className="space-y-1.5">
                    {[
                      ...
                      >
                        <span className={`flex-shrink-0 ${primary ? 'text-gold' : 'text-gold/60 group-hover:text-gold/90'}`}>{icon}</span>
                        <div className="flex-1 min-w-0">
              ...
              {!isActive && (
                <Link
                  ...
                  style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.12) 0%, rgba(10,10,15,0.97) 65%)', boxShadow: '0 0 24px rgba(212,175,55,0.08)' }}
                >
                  <div className="p-5">
            ...
            <div className="mt-12 space-y-6">
              ...
              <div className="card-surface p-6 rounded-xl">
                ...
                {inviteLinks.length > 0 ? (
                        ...
                        <div key={link.id} className="flex items-center gap-3 p-3 rounded-xl bg-bg-hover border border-white/5">
                          ...
                          <div className="flex items-center gap-3 flex-shrink-0">
                            <div className="flex items-center gap-1 text-xs text-arc-muted">
                              <Eye className="w-3 h-3" />{link.click_count}
                            </div>
                            <ActionButton
                  ...
                  <p className="text-xs text-arc-muted text-center py-6">No links yet.</p>
                )}
              </div>
            </div>
          )}
